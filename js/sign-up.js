...
function validateUsername(){
    ...
    if(re.test(username.value)){
        ...
        if((/(\b\d|\d\b)/).test(username.value)){
            displayErrorMessage(username , '<div>username must start with a letter and end with a letter')
        }
        ...
        if(username.value.length > 16 || username.value.length < 5 ){
            displayErrorMessage(username , 'username must 5 character at least and 15 character at most ')
        }
    }
}
